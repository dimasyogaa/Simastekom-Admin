...
fun newPasswordSameWithNewConfirmPassword(
    ...
    edtPassword.addTextChangedListener(object : TextWatcher {
        ...
        override fun afterTextChanged(p0: Editable?) {
            if (newPasswordHelper == newConfirmPasswordHelper) {
                newPasswordSameWithNewConfirmPassword(context, newPasswordHelper, p0.toString(), layout)
            } else {
                newPasswordSameWithNewConfirmPassword(context, newPasswordHelper, p0.toString(), layout)
            }
        }
    ...
    return newPasswordSameWithNewConfirmPassword(context, newPasswordHelper, password, layout, true)
}
...
fun newPasswordSameWithNewConfirmPassword(
    ...
    if (password.isNotEmpty()) {
        if (password == newPassword) {
            ...
            layout.helperText =
                context.getString(R.string.text_confirm_the_new_password_matches_the_new_password_you_entered)
            isValid = true
        } else {
            layout.isErrorEnabled = true
            layout.error =
