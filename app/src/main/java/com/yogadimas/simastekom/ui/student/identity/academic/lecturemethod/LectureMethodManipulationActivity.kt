...
import com.yogadimas.simastekom.databinding.ActivityLectureMethodManipulationBinding
...
class LectureMethodManipulationActivity : AppCompatActivity() {
    private lateinit var binding: ActivityLectureMethodManipulationBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLectureMethodManipulationBinding.inflate(layoutInflater)
    ...
    private fun getAdminAndData() {
        ...
        authViewModel.getUser().observe(this) {
            ...
            if (token == AuthPreferences.DEFAULT_VALUE) {
                ...
                if (isEditDeleteView) {
                    adminViewModel.getLectureMethodById(id)
                }
            }
        }
    ...
    private fun save() {
        binding.apply {
            hideKeyboard()
            if (isEditDeleteView) {
                adminViewModel.updateLectureMethod(
                    id,
                    NameData(
                ...
                adminViewModel.addLectureMethod(
                    NameData(
                        id = id,
                        name = edtName.text.toString().trim(),
                    )
            ...
            dialog = MaterialAlertDialogBuilder(this).apply {
                ...
                if (status == STATUS_DELETED || status == STATUS_ERROR) {
                    setPositiveButton(getString(R.string.text_ok)) { _, _ ->
                        ...
                        dialog = null
                        when (status) {
                            STATUS_DELETED -> adminViewModel.deleteLectureMethod(id)
                            STATUS_ERROR -> {
