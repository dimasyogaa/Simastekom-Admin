...
class EmploymentStatusManipulationActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEmploymentStatusManipulationBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEmploymentStatusManipulationBinding.inflate(layoutInflater)
    ...
    private fun getAdminAndData() {
        ...
        authViewModel.getUser().observe(this) {
            ...
            if (token == AuthPreferences.DEFAULT_VALUE) {
                ...
                if (isEditDeleteView) {
                    adminViewModel.getEmploymentStatusById(id)
                }
            }
        }
    ...
    private fun save() {
        binding.apply {
            hideKeyboard()
            if (isEditDeleteView) {
                adminViewModel.updateEmploymentStatus(
                    id,
                    NameData(
                ...
                adminViewModel.addEmploymentStatus(
                    NameData(
                        id = id,
                        name = edtName.text.toString().trim(),
                    )
            ...
            dialog = MaterialAlertDialogBuilder(this).apply {
                ...
                if (status == STATUS_DELETED || status == STATUS_ERROR) {
                    setPositiveButton(getString(R.string.text_ok)) { _, _ ->
                        ...
                        dialog = null
                        when (status) {
                            STATUS_DELETED -> adminViewModel.deleteEmploymentStatus(id)
                            STATUS_ERROR -> {
