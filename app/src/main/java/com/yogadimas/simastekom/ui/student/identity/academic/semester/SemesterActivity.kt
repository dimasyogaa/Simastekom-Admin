package com.yogadimas.simastekom.ui.student.identity.academic.semester
...
class SemesterActivity : AppCompatActivity() {
    ...
    private val resultLauncher = registerForActivityResult(
        ...
        if (result.resultCode == SemesterManipulationActivity.KEY_RESULT_CODE && result.data != null) {
            val successText =
                result.data?.getStringExtra(SemesterManipulationActivity.KEY_EXTRA_SUCCESS)
                    .orEmpty()
    ...
    private fun mainCall() {
        binding.apply {
            ...
            listOf(chipSortBy).forEach { chipId ->
            ...
            val layoutManager = LinearLayoutManager(this@SemesterActivity)
            rvSemester.layoutManager = layoutManager
    ...
    private fun getAdminAndData() {
        ...
        authViewModel.getUser().observe(this) {
            ...
            if (token == AuthPreferences.DEFAULT_VALUE) {
                ...
                adminViewModel.getAllSemesters()
            }
        }
        ...
        adminViewModel.name.observe(this) { eventData ->
            eventData.getContentIfNotHandled()?.let {
                ...
                if (it.isDeleted) {
                    ...
                    val label = getString(R.string.text_label_semester)
                    showAlertDialog(
            ...
            dialog = MaterialAlertDialogBuilder(this).apply {
                ...
                if (status == STATUS_DELETED || status == STATUS_ERROR) {
                    setPositiveButton(getString(R.string.text_ok)) { _, _ ->
                        ...
                        dialog = null
                        when (status) {
                            STATUS_DELETED -> adminViewModel.deleteSemester(
                                id
                            )
    ...
    private fun isVisibleAllView(boolean: Boolean) {
        binding.apply {
            ...
            if (boolean) {
                ...
                rvSemester.visibility = View.GONE
            }
        }
    }
    ...
    private fun showMenu(v: View, menuRes: Int, sortBy: String) {
        val popup = PopupMenu(this@SemesterActivity, v)
