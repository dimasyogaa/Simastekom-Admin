package com.yogadimas.simastekom.ui.student.status.employment
...
import com.yogadimas.simastekom.databinding.ActivityEmploymentStatusBinding
...
class EmploymentStatusActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEmploymentStatusBinding
    ...
    private val resultLauncher = registerForActivityResult(
        ...
        if (result.resultCode == EmploymentStatusManipulationActivity.KEY_RESULT_CODE && result.data != null) {
            val successText =
                result.data?.getStringExtra(EmploymentStatusManipulationActivity.KEY_EXTRA_SUCCESS)
                    .orEmpty()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEmploymentStatusBinding.inflate(layoutInflater)
        ...
        binding.apply {
            ...
            searchView
                ...
                .setOnEditorActionListener { _, _, _ ->
                    ...
                    adminViewModel.searchSortEmploymentStatus(
            ...
            fabAdd.setOnClickListener {
                resultLauncher.launch(
                    Intent(
                        this@EmploymentStatusActivity,
                        EmploymentStatusManipulationActivity::class.java
                    )
                )
            }

            val layoutManager = LinearLayoutManager(this@EmploymentStatusActivity)
            rvEmploymentStatus.layoutManager = layoutManager
    ...
    private fun getAdminAndData() {
        ...
        authViewModel.getUser().observe(this) {
            ...
            if (token == AuthPreferences.DEFAULT_VALUE) {
                ...
                adminViewModel.getAllEmploymentStatuses()
            }
        }
        ...
        adminViewModel.name.observe(this) { eventData ->
            eventData.getContentIfNotHandled()?.let {
                ...
                if (it.isDeleted) {
                    ...
                    val label = getString(R.string.text_label_employment_status)
                    showAlertDialog(
    ...
    private fun setData(it: List<NameData>) {
        val adapter =  if (isFromStudent) {
            NameAdapter(object : OnItemClickManipulationCallback<NameData> {
                override fun onItemClicked(data: NameData) {
                    ...
                    val studentData = StudentData(
                        employmentStatusId = data.id,
                        employmentStatusName = data.name,
                    )
            ...
            override fun onItemClicked(data: NameData) {
                val intent = Intent(
                    ...
                    putExtra(EmploymentStatusManipulationActivity.KEY_EXTRA_ID, data.id)
                }
        ...
        binding.rvEmploymentStatus.adapter = adapter
    }
            ...
            dialog = MaterialAlertDialogBuilder(this).apply {
                ...
                if (status == STATUS_DELETED || status == STATUS_ERROR) {
                    setPositiveButton(getString(R.string.text_ok)) { _, _ ->
                        ...
                        dialog = null
                        when (status) {
                            STATUS_DELETED -> adminViewModel.deleteEmploymentStatus(
                                id
                            )
    ...
    private fun isVisibleAllView(boolean: Boolean) {
        binding.apply {
            ...
            if (boolean) {
                ...
                rvEmploymentStatus.visibility = View.VISIBLE

            } else {
                toolbar.visibility = View.INVISIBLE
                toolbar2.visibility = View.GONE
                fabAdd.visibility = View.GONE
                rvEmploymentStatus.visibility = View.GONE
            }
        }
    }
    ...
    private fun showMenu(v: View, menuRes: Int, sortBy: String) {
        val popup = PopupMenu(this@EmploymentStatusActivity, v)
    ...
    companion object {
        ...
        const val KEY_STUDENT_RESULT_CODE = 11_500
