...
class StudentStatusManipulationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityStudentStatusManipulationBinding.inflate(layoutInflater)
    ...
    private fun getAdminAndData() {
        ...
        authViewModel.getUser().observe(this) {
            ...
            if (token == AuthPreferences.DEFAULT_VALUE) {
                ...
                if (isEditDeleteView) {
                    adminViewModel.getStudentStatusById(id)
                }
            }
        }
    ...
    private fun save() {
        binding.apply {
            hideKeyboard()
            if (isEditDeleteView) {
                adminViewModel.updateStudentStatus(
                    id,
                    NameData(
                ...
                adminViewModel.addStudentStatus(
                    NameData(
                        id = id,
                        name = edtName.text.toString().trim(),
                    )
            ...
            dialog = MaterialAlertDialogBuilder(this).apply {
                ...
                if (status == STATUS_DELETED || status == STATUS_ERROR) {
                    setPositiveButton(getString(R.string.text_ok)) { _, _ ->
                        ...
                        dialog = null
                        when (status) {
                            STATUS_DELETED -> adminViewModel.deleteStudentStatus(id)
                            STATUS_ERROR -> {
